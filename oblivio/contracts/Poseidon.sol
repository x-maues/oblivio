...
contract Poseidon {
    ...
    uint256 constant t = 3; // Increased to handle 4 inputs + 1 for padding
    ...
    function hash(uint256 x, uint256 y) public view returns (uint256) {
        uint256[3] memory state = [x % p, y % p, uint256(0)]; // Add a padding zero
        ...
        return state[0]; // Return first element as hash
    }
